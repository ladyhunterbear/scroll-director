 ...
 *
 */
var ScrollDirector = /** @class */ (function () {
    /**
    ...
    function ScrollDirector() {
        this.el = window;
        this.minScrollTime = 20; // milliseconds between running through trigger queue.
        this.minScrollAmount = 10; // distance in pixels before running through trigger queue.
     ...
     *
     */
    ScrollDirector.prototype.inViewport = function (trigger) {
        var status;
        if (trigger.top() < window.innerHeight && trigger.bottom() > 0) {
            status = enums_1.ViewportPosition["in"];
        }
        else if (trigger.bottom() <= 0) {
            status = enums_1.ViewportPosition.before;
        }
        else if (trigger.top() >= window.innerHeight) {
            status = enums_1.ViewportPosition.after;
        }
        return status;
    };
    /**
     ...
     *
     */
    ScrollDirector.prototype.checkTriggers = function () {
     ...
     * @return Trigger
     */
    ScrollDirector.prototype.add = function (element) {
        var trigger = new Triggers_1.Trigger(element);
        if (trigger)
            this.triggers.push(trigger);
        var i = this.triggers.length - 1;
     ...
     *
     */
    ScrollDirector.prototype.draw = function () {
        var _this = this;
        setTimeout(function () {
   ...
   * trigger conditions.
     *
     */
    ScrollDirector.prototype.resize = function () {
     ...
     *
     */
    ScrollDirector.prototype.init = function () {
        var _this = this;
        try {
            ...
            window.addEventListener('scroll', function () {
                setTimeout(function () { _this.checkTriggers(); }, 200);
                _this.checkTriggers();
            });
...
exports["default"] = ScrollDirector;
